...
EXPERIMENT_TITLE = "Calibration-Aware Gradient-Directed Matching (CA-GDM) TinyLlama white-box pilot"
THREAT_MODEL = (
...
CG_METHODS = {"cagdm", "cagdm_bos_debias"}
...
ALPHA_INIT_METHODS = {"alpha_nn_init_residual", "alpha_nn_global_gradmatch", "cagdm", "cagdm_bos_debias"}
CONTEXT_METHODS = {"attention_context_existing", "alpha_nn_global_gradmatch", "cagdm", "cagdm_bos_debias"}



@dataclass
class CAGDMDiagnosticConfig:
    top_m: int = 3
    force_gate_closed: bool = False
    tau_consensus: float = 0.6
    tau_grad_margin: float = 0.03
    tau_entropy: float = 0.3
    threshold_cal: float = 0.005
...
def candidate_entropy(scores: torch.Tensor) -> torch.Tensor:
    if scores.numel() <= 1:
        return torch.tensor(0.0, device=scores.device)
    probs = torch.softmax(scores.float(), dim=0)
    return -(probs.clamp_min(1e-12) * probs.clamp_min(1e-12).log()).sum()


def gradient_direction_scores(candidate_dirs: torch.Tensor, grad_dir: torch.Tensor, mode: str) -> torch.Tensor:
    mode = mode.lower()
    if grad_dir.norm() <= 0:
        return torch.full((candidate_dirs.shape[0],), -1.0, device=candidate_dirs.device)
    if mode == 'cosine':
        return F.cosine_similarity(candidate_dirs.float(), grad_dir.float().unsqueeze(0).expand_as(candidate_dirs), dim=-1)
    if mode == 'dot':
        return torch.matmul(candidate_dirs.float(), grad_dir.float())
    raise ValueError(f'Unknown gradient_score={mode!r}')


def resolve_calibration_margin_threshold(mode: str, margins: Sequence[float]) -> float:
    finite = [float(x) for x in margins if math.isfinite(float(x))]
    if not finite:
        return float('inf')
    mode = str(mode)
    table = {'prompt_quantile_20': 0.20, 'prompt_quantile_30': 0.30, 'prompt_quantile_50': 0.50}
    if mode in table:
        return float(np.quantile(finite, table[mode]))
    if mode.startswith('fixed_'):
        return float(mode.split('_', 1)[1])
    return float(mode)


def should_open_gradient_gate(
    ...
    reasons: List[str] = []
    if cfg.force_gate_closed:
        reasons.append('force_gate_closed')
    if cfg.top_m <= 1:
        reasons.append('top_m_le_1')
    if distinct_token_count < 2:
        reasons.append('not_enough_distinct_tokens')
    if kappa < cfg.tau_consensus:
        reasons.append('kappa_below_tau')
    if grad_margin < cfg.tau_grad_margin:
        reasons.append('grad_margin_below_tau')
    if cal_margin > cfg.threshold_cal:
        reasons.append('calibration_confident')
    if entropy < cfg.tau_entropy:
        reasons.append('entropy_below_tau')
    return GateDecision(gate_open=len(reasons) == 0, closed_reasons=reasons)
...
def cagdm_rerank(
    ...
    tau_cal_margin = resolve_calibration_margin_threshold(cfg.threshold_cal, prompt_cal_margins)
    recovered = naive_discretization(z, embed_layer.weight)
    for pos, token_id in fixed_public.items():
        if 0 <= pos < len(recovered):
            recovered[pos] = int(token_id)
    ...
    for pos in range(seq_len):
        ...
        grad_scores = gradient_direction_scores(candidate_dirs, grad_dir, cfg.gradient_score)
        ...
        entropy_val = float(candidate_entropy(scores[cal_top]).detach().cpu())
        gate_decision = should_open_gradient_gate(
            CAGDMDiagnosticConfig(
                top_m=top_m,
                force_gate_closed=force_gate_closed,
                tau_consensus=cfg.tau_consensus,
                tau_grad_margin=cfg.tau_grad_margin,
                tau_entropy=cfg.tau_entropy,
                threshold_cal=tau_cal_margin,
            ),
            kappa=kappa,
            grad_margin=grad_margin,
            cal_margin=cal_margin,
            entropy=entropy_val,
            distinct_token_count=len(set(candidate_subset)),
        )
        gradient_reliable = gate_decision.gate_open
        ...
        gate_rows.append(
            {
                ...
                "tau_grad_margin": cfg.tau_grad_margin,
                "tau_entropy": cfg.tau_entropy,
                "candidate_entropy": entropy_val,
                "threshold_cal_mode": cfg.threshold_cal,
                "closed_reasons": gate_decision.closed_reasons,
                "gradient_score_mode": cfg.gradient_score,
...
def invert_observed(
    ...
    if method in STAGE_A_METHODS:
        ...
        mode = "bos_debias" if method == "cagdm_bos_debias" else cfg.attention_mode
    ...
    if method in CG_METHODS:
        if h_init is None or not init_ids:
            raise RuntimeError("CA-GDM requires residual alpha-NN initialization")
        recovered_ids, calibration_score, gate_audit = cagdm_rerank(
...
def validate_attack_api() -> Dict[str, Any]:
    ...
    for fn in checked:
        ...
        if fn is cagdm_rerank and ("minmax01" in source or "old_minmax" in source):
...
def config_from_args(args: argparse.Namespace, method: str, seed: int, run_name: str, output_dir: str) -> CGConfig:
    ...
    attention_mode = "bos_debias" if method == "cagdm_bos_debias" else args.attention_mode
    return CGConfig(
        method=method,
        run_name=run_name,
        ...
        gradient_score=args.gradient_score,
        threshold_cal=args.threshold_cal,
...
def run_verify(args: argparse.Namespace) -> None:
    ...
    base = config_from_args(args, "cagdm", args.seed, "verify", str(out / "verify"))
    ...
    cfg_top1 = config_from_args(args, "cagdm", args.seed, "verify_top1", str(out / "verify_top1"))
    ...
    cfg_closed = config_from_args(args, "cagdm", args.seed, "verify_closed", str(out / "verify_closed"))
    ...
    result = {
        ...
        "attack_api_check": api_check,
        "randomized_eval_text": "Random evaluation-only text that must not affect CA-GDM attack output.",
    ...
    if not (
        ...
        raise RuntimeError(f"CA-GDM leakage/unit verification failed: {result}")
...
def write_plots(root: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    if valid:
        ...
        ax.set_title("CA-GDM token accuracy comparison")
        ...
        ax.set_ylim(0, 1)
        ax.set_ylabel("BLEU")
        ax.set_title("CA-GDM BLEU comparison")
        ax.set_xticks(range(len(valid)), labels=labels, rotation=75, ha="right", fontsize=7)
        fig.tight_layout()
    ...
    for filename, title, data in [
        ("gate_open_rate.png", "CA-GDM gate open rate", gate_points),
        ("override_accuracy_delta.png", "CA-GDM override rate", override_points),
    ]:
        if data:
            labels, ys = zip(*data)
...
def write_report(root: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "## Method",
        "",
        "CA-GDM keeps residual alpha-NN initialization and replaces global min-max gradient fusion with calibration-first top-M reranking guarded by gradient consensus.",
        "",
    ...
    lines.extend(
        [
            ...
            "- CA-GDM only allows gradients to rerank calibration top-M candidates when consensus and margin checks pass.",
    ...
    (analysis_dir / "cagdm_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--output-root", default="runs/cagdm")
    ...
    parser.add_argument("--gradient-score", choices=["cosine", "dot"], default="cosine")
    parser.add_argument("--threshold-cal", default="prompt_quantile_30")
    parser.add_argument("--tau-entropy", type=float, default=0.3)
