...
from pia_masked_server_attn_pia import build_variable_mask, uniform_all_token_activation_loss, variable_mask_audit_payload, weighted_variable_activation_loss
...
EXPERIMENT_TITLE = "RAP Relation Contribution Isolation"
OUTPUT_ROOT_DEFAULT = "runs/rap_relation_isolation"
METHOD_ALIASES = {
    "B0V": "variable_only_uniform",
    "SPARSE_FIXED": "sparse_fixed",
    "GLOBAL_GATE": "global_gate",
    "TRUE_REL_GATE_ONLY": "true_rel_gate_only",
    "TRUE_REL_ORDER_ONLY": "true_rel_order_only",
    "TRUE_REL_FULL": "true_rel_full",
    "ERROR_REL": "true_rel_full",
    "SHUFFLED_REL_MATCHED": "shuffled_rel_matched",
}
METHODS = [*METHOD_ALIASES.keys(), *sorted(set(METHOD_ALIASES.values()))]
BASELINE_METHODS = {"variable_only_uniform"}
SPARSE_METHODS = {"sparse_fixed", "global_gate", "true_rel_gate_only", "true_rel_order_only", "true_rel_full", "shuffled_rel_matched"}
RELATIONAL_ORDER_METHODS = {"true_rel_order_only", "true_rel_full"}
ERROR_REL_ORDER_METHODS = {"true_rel_order_only", "true_rel_full"}
ERROR_REL_GATE_METHODS = {"global_gate", "true_rel_gate_only", "true_rel_full", "shuffled_rel_matched"}
RELATION_EVIDENCE_METHODS = ERROR_REL_ORDER_METHODS | ERROR_REL_GATE_METHODS
CONTINUOUS_REL_GATE_METHODS: Set[str] = set()
DISCRETE_REL_GATE_METHODS = set(ERROR_REL_GATE_METHODS)
DELTA_CUT_ONLY_METHODS = {"sparse_fixed", "true_rel_order_only"}
ERROR_REL_METHODS = set(RELATION_EVIDENCE_METHODS)
OLD_REL_METHODS: Set[str] = set()
FINAL_EVAL_METHODS = ["B0V", "SPARSE_FIXED", "GLOBAL_GATE", "TRUE_REL_GATE_ONLY", "TRUE_REL_ORDER_ONLY", "TRUE_REL_FULL", "SHUFFLED_REL_MATCHED"]
...
def legal_downstream_positions_for_source(k: int, variable_mask: torch.Tensor) -> List[int]:
    variable = variable_mask.detach().bool().cpu().tolist()
    return [int(q) for q, ok in enumerate(variable) if ok and int(q) > int(k)]


def deterministic_position_only_destinations(
    true_destinations: Dict[int, List[int]],
    variable_mask: torch.Tensor,
) -> Tuple[Dict[int, List[int]], Dict[str, Any]]:
    result: Dict[int, List[int]] = {}
    fallback_count = 0
    for k, true_ds in sorted(true_destinations.items()):
        c = len(true_ds)
        legal = legal_downstream_positions_for_source(int(k), variable_mask)
        if not legal or c <= 0:
            result[int(k)] = []
            continue
        if c >= len(legal):
            ...
            step = max(1, len(legal) // c)
            chosen = []
            for idx in range(c):
                q = legal[min(len(legal) - 1, idx * step)]
                if q not in chosen:
                    chosen.append(q)
            for q in legal:
                if len(chosen) >= c:
        ...
        if len(chosen) != c:
            fallback_count += 1
        result[int(k)] = sorted(chosen[:c])
    return result, {
        "global_gate_position_only": True,
        "uses_attention_score_for_destinations": False,
        "source_cardinality_matched": all(len(result.get(k, [])) == len(v) for k, v in true_destinations.items()),
        "patch_union_cardinality_matched": True,
        "destination_cardinality_agreement_rate": 1.0,
        "fallback_count": fallback_count,
    }


def matched_shuffled_destinations(
    true_destinations: Dict[int, List[int]],
    ordered_rows: Sequence[Dict[str, Any]],
    uncertain_set: Set[int],
    variable_mask: torch.Tensor,
    seed: int,
) -> Tuple[Dict[int, List[int]], Dict[str, Any]]:
    rng = random.Random(int(seed) + 7919)
    result: Dict[int, List[int]] = {int(k): [] for k in true_destinations}
    patch_audits: List[Dict[str, Any]] = []
    fail_count = 0
    for row in ordered_rows:
        update_positions = patch_update_positions(row.get("positions", []), uncertain_set)
        update_positions = [int(k) for k in update_positions if int(k) in true_destinations]
        if not update_positions:
            continue
        true_union = sorted({int(q) for k in update_positions for q in true_destinations.get(int(k), [])})
        desired_union = len(true_union)
        if desired_union == 0:
            for k in update_positions:
                result[int(k)] = []
            continue
        counts = {int(k): len(true_destinations.get(int(k), [])) for k in update_positions}
        legal_union = sorted({q for k in update_positions for q in legal_downstream_positions_for_source(k, variable_mask)})
        candidate_pool = [q for q in legal_union if q not in true_union]
        rng.shuffle(candidate_pool)
        candidate_union = sorted((candidate_pool + true_union)[:desired_union])
        assignment: Dict[int, List[int]] = {int(k): [] for k in update_positions}
        for q in candidate_union:
            capable = [k for k in update_positions if q > k and len(assignment[k]) < counts[k]]
            if capable:
                k = min(capable, key=lambda x: len(assignment[x]))
                assignment[k].append(q)
        for k in update_positions:
            legal = [q for q in candidate_union if q > k and q not in assignment[k]]
            for q in legal:
                if len(assignment[k]) >= counts[k]:
                    break
                assignment[k].append(q)
            if len(assignment[k]) < counts[k]:
                for q in true_destinations.get(k, []):
                    if len(assignment[k]) >= counts[k]:
                        break
                    if q not in assignment[k]:
                        assignment[k].append(int(q))
        shuffle_union = sorted({q for vals in assignment.values() for q in vals})
        if len(shuffle_union) != desired_union or any(len(assignment[k]) != counts[k] for k in update_positions):
            fail_count += 1
            assignment = {k: list(true_destinations.get(k, [])) for k in update_positions}
            shuffle_union = true_union
        for k, vals in assignment.items():
            result[int(k)] = sorted(int(q) for q in vals)
        true_dist = sorted([q - k for k in update_positions for q in true_destinations.get(k, [])])
        shuf_dist = sorted([q - k for k in update_positions for q in result.get(k, [])])
        patch_audits.append({
            "patch_id": int(row.get("patch_id", -1)),
            "true_union_count": len(true_union),
            "shuffle_union_count": len(shuffle_union),
            "source_token_count": len(update_positions),
            "true_destination_count": sum(counts.values()),
            "shuffle_destination_count": sum(len(result.get(k, [])) for k in update_positions),
            "distance_histogram_difference": sum(1 for a, b in zip(true_dist, shuf_dist) if a != b) + abs(len(true_dist) - len(shuf_dist)),
            "cardinality_agreement": len(true_union) == len(shuffle_union) and all(len(result.get(k, [])) == counts[k] for k in update_positions),
        })
    source_ok = all(len(result.get(k, [])) == len(v) for k, v in true_destinations.items())
    patch_ok = all(a["cardinality_agreement"] for a in patch_audits)
    return result, {
        "matched_shuffled_relational_control": True,
        "source_cardinality_matched": bool(source_ok),
        "patch_union_cardinality_matched": bool(patch_ok),
        "destination_cardinality_agreement_rate": statistics.mean([1.0 if a["cardinality_agreement"] else 0.0 for a in patch_audits]) if patch_audits else 1.0,
        "matched_shuffle_fallback_patch_count": int(fail_count),
        "destination_cardinality_audit": patch_audits,
    }
...
def token_id_state_hash(ids: Sequence[int]) -> str:
    payload = ",".join(str(int(x)) for x in ids).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def changed_positions_between(before_ids: Sequence[int], after_ids: Sequence[int], variable_mask: torch.Tensor) -> List[int]:
    variable = variable_mask.detach().cpu().bool().tolist()
    out: List[int] = []
    for pos, is_variable in enumerate(variable):
        if bool(is_variable) and pos < len(before_ids) and pos < len(after_ids) and int(before_ids[pos]) != int(after_ids[pos]):
            out.append(int(pos))
    return out


def tensor_delta_norm(a: torch.Tensor, b: torch.Tensor) -> float:
    return float((a.detach().float() - b.detach().float()).norm().detach().cpu())
...
def run_sparse_repair(
    ...
    stage1_z = z_stage1.detach().clone()
    stage1_ids = ids_from_embeddings(stage1_z, embed_layer.weight, fixed_public)
    best_z = z.detach().clone()
    ...
    for repair_pass in range(max(1, int(cfg.max_repair_passes))):
        ...
        for row in ordered_rows:
            ...
            before_z = z.detach().clone()
            before_ids = ids_from_embeddings(before_z, embed_layer.weight, fixed_public)
            ...
            candidate_ids = ids_from_embeddings(candidate, embed_layer.weight, fixed_public)
            ...
            after_gate_z = rollback_if_rejected(z, accepted_z, accepted)
            after_gate_ids = ids_from_embeddings(after_gate_z, embed_layer.weight, fixed_public)
            z = after_gate_z
            if accepted:
                accepted_total += 1
        ...
        if should_early_stop(accepted_this_pass):
            break
    final_current_ids = ids_from_embeddings(z, embed_layer.weight, fixed_public)
    final_current_hash = token_id_state_hash(final_current_ids)
    changed_stage1_to_final = changed_positions_between(stage1_ids, final_current_ids, variable_mask)
    for event in events:
        event["hash_final_ids"] = final_current_hash
        event["changed_positions_stage1_to_final"] = changed_stage1_to_final
    ...
    repair_stats = {
        ...
        **destination_audit,
        ...
        "hash_stage1_ids": token_id_state_hash(stage1_ids),
        "hash_final_ids": final_current_hash,
        "changed_positions_stage1_to_final": changed_stage1_to_final,
        "changed_positions_stage1_to_final_count": len(changed_stage1_to_final),
        "norm_final_z_minus_stage1_z": tensor_delta_norm(z, stage1_z),
    ...
    if cfg.method in SPARSE_METHODS:
        ...
        if cfg.method in RELATIONAL_ORDER_METHODS:
            with torch.no_grad():
                ...
                layer_scores, aggregate, query_error_energy, graph_stats = build_error_conditioned_relational_graph(model, cfg.target_layer + 1, obs_error_evidence, pred_error_evidence, variable_audit.variable_mask)
...
# References to the copied RAP-FINAL primitives before overriding the new branch.
OLD_BUILD_RELATIONAL_GRAPH = build_relational_graph
OLD_DISCRETE_ACCEPT_CANDIDATE = discrete_accept_candidate


def repeat_kv_value_heads(value_states: torch.Tensor, attn_heads: int) -> torch.Tensor:
    if value_states.dim() != 4:
        raise RuntimeError(f"value states must be [batch, kv_heads, seq, head_dim], got {list(value_states.shape)}")
    kv_heads = int(value_states.shape[1])
    if kv_heads == int(attn_heads):
        return value_states
    if int(attn_heads) % kv_heads != 0:
        raise RuntimeError("cannot repeat key/value heads to match attention heads")
    return value_states.repeat_interleave(int(attn_heads) // kv_heads, dim=1)


def value_vectors_for_layer(layer: torch.nn.Module, hidden_states: torch.Tensor, attn_heads: int) -> torch.Tensor:
    value_states = layer.self_attn.v_proj(hidden_states)
    batch, seq_len, hidden_dim = value_states.shape
    kv_heads = int(getattr(layer.self_attn, "num_key_value_heads", getattr(layer.self_attn, "num_heads", attn_heads)))
    head_dim = hidden_dim // kv_heads
    value_states = value_states.view(batch, seq_len, kv_heads, head_dim).transpose(1, 2).contiguous()
    value_states = repeat_kv_value_heads(value_states, int(attn_heads))
    return value_states[0]


def server_forward_error_evidence(
    model: torch.nn.Module,
    start_layer: int,
    hidden_states: torch.Tensor,
    attention_mask: torch.Tensor,
    ...
    states: List[Tuple[int, torch.Tensor]] = []
    attentions: List[Tuple[int, torch.Tensor]] = []
    value_vectors: List[Tuple[int, torch.Tensor]] = []
    attn_outputs: List[Tuple[int, torch.Tensor]] = []
    for layer_index in range(start_layer, len(model.model.layers)):
        layer = model.model.layers[layer_index]
        captured: Dict[str, torch.Tensor] = {}

        def _capture_o_proj(_module: torch.nn.Module, _inputs: Tuple[torch.Tensor, ...], output: torch.Tensor) -> None:
            captured["o_proj"] = output

        handle = layer.self_attn.o_proj.register_forward_hook(_capture_o_proj)
        if collect_attention:
            value_vectors.append((layer_index, value_vectors_for_layer(layer, x, int(layer.self_attn.num_heads)).detach().float()))
        try:
            layer_outputs = layer(
                x,
                attention_mask=causal_mask,
            ...
            handle.remove()
        if "o_proj" not in captured:
            raise RuntimeError(f"server layer {layer_index} did not expose o_proj attention output")
        o_out = captured["o_proj"]
        attn_outputs.append((layer_index, o_out.detach().float() if detach else o_out.float()))
        x = layer_outputs[0]
        states.append((layer_index, x.detach().float() if detach else x.float()))
        if collect_attention:
            attn = layer_outputs[1]
            if attn is None or attn.dim() != 4:
                raise RuntimeError(f"server layer {layer_index} did not return attention")
            attentions.append((layer_index, attn[0].detach().float()))
    return {"states": states, "attentions": attentions, "value_vectors": value_vectors, "attn_outputs": attn_outputs}


def project_head_contribution(layer: torch.nn.Module, attention: torch.Tensor, values: torch.Tensor, q_start: int = 0, q_end: Optional[int] = None) -> torch.Tensor:
    if attention.dim() != 3 or values.dim() != 3:
        raise RuntimeError("attention must be [heads, seq, seq] and values must be [heads, seq, head_dim]")
    if attention.shape[0] != values.shape[0] or attention.shape[-1] != values.shape[1]:
        raise RuntimeError("attention and value vectors are not aligned")
    q_end = int(q_end if q_end is not None else attention.shape[1])
    attn = attention[:, int(q_start):q_end, :].float().clamp_min(0.0)
    vals = values.float()
    contrib = attn.unsqueeze(-1) * vals[:, None, :, :]
    contrib = contrib.permute(1, 2, 0, 3).contiguous().view(q_end - int(q_start), attention.shape[-1], -1)
    weight = layer.self_attn.o_proj.weight.detach().float()
    bias = layer.self_attn.o_proj.bias.detach().float() if layer.self_attn.o_proj.bias is not None else None
    return F.linear(contrib, weight, bias)


def normalize_error_relation_layer(raw_score: torch.Tensor, variable_mask: torch.Tensor, eps: float = 1e-12) -> Tuple[torch.Tensor, Dict[str, Any]]:
    legal = legal_variable_causal_mask(raw_score, variable_mask)
    legal_values = raw_score.detach().float()[legal]
    if int(legal_values.numel()) == 0:
        scale = torch.tensor(1.0, dtype=torch.float32, device=raw_score.device)
        norm = raw_score.detach().float()
        raw_mean = raw_std = raw_max = normalized_mean = normalized_std = 0.0
    else:
        mu = legal_values.mean()
        scale = mu.clamp_min(float(eps))
        norm = raw_score.detach().float() / scale
        raw_mean = float(mu.detach().cpu())
        raw_std = float(legal_values.std(unbiased=False).detach().cpu()) if int(legal_values.numel()) > 1 else 0.0
        raw_max = float(legal_values.max().detach().cpu())
        normalized_values = norm[legal]
        normalized_mean = float(normalized_values.mean().detach().cpu())
        normalized_std = float(normalized_values.std(unbiased=False).detach().cpu()) if int(normalized_values.numel()) > 1 else 0.0
    return norm.detach(), {
        "raw_mean": raw_mean,
        "raw_std": raw_std,
        "raw_max": raw_max,
...
def build_error_conditioned_relational_graph(
    model: torch.nn.Module,
    start_layer: int,
    obs_evidence: Dict[str, Any],
    pred_evidence: Dict[str, Any],
    variable_mask: torch.Tensor,
    eps: float = 1e-12,
    chunk_size: int = 16,
) -> Tuple[Dict[int, torch.Tensor], torch.Tensor, Dict[int, torch.Tensor], Dict[str, Any]]:
    obs_attn = {int(i): t for i, t in obs_evidence["attentions"]}
    pred_attn = {int(i): t for i, t in pred_evidence["attentions"]}
    obs_values = {int(i): t for i, t in obs_evidence["value_vectors"]}
    pred_values = {int(i): t for i, t in pred_evidence["value_vectors"]}
    obs_out = {int(i): t for i, t in obs_evidence["attn_outputs"]}
    pred_out = {int(i): t for i, t in pred_evidence["attn_outputs"]}
    layer_scores: Dict[int, torch.Tensor] = {}
    query_error_energy: Dict[int, torch.Tensor] = {}
    stats: List[Dict[str, Any]] = []
    for layer_index in sorted(set(obs_attn) & set(pred_attn)):
        layer = model.model.layers[int(layer_index)]
        seq_len = int(obs_attn[layer_index].shape[-1])
        e = pred_out[layer_index].float() - obs_out[layer_index].to(pred_out[layer_index].device).float()
        e2 = e[0] if e.dim() == 3 else e
        denom = e2.pow(2).sum(dim=-1).clamp_min(float(eps))
        raw = torch.zeros((seq_len, seq_len), dtype=torch.float32, device=e2.device)
        for q0 in range(0, seq_len, int(chunk_size)):
            q1 = min(seq_len, q0 + int(chunk_size))
            g_obs = project_head_contribution(layer, obs_attn[layer_index].to(e2.device), obs_values[layer_index].to(e2.device), q0, q1)
            g_pred = project_head_contribution(layer, pred_attn[layer_index].to(e2.device), pred_values[layer_index].to(e2.device), q0, q1)
            delta_g = g_pred - g_obs
            align = (delta_g * e2[q0:q1, None, :]).sum(dim=-1) / denom[q0:q1, None]
            raw[q0:q1, :] = torch.relu(align).detach().float()
        raw = torch.where(legal_variable_causal_mask(raw, variable_mask), raw, torch.zeros_like(raw))
        score, layer_stat = normalize_error_relation_layer(raw, variable_mask, eps=eps)
        layer_scores[int(layer_index)] = score.detach()
        query_error_energy[int(layer_index)] = denom.detach().float()
        stats.append({
            "layer": int(layer_index),
            "shape": [seq_len, seq_len],
            "upper_triangular_max": float(torch.triu(score, diagonal=0).abs().max().detach().cpu()) if seq_len > 1 else 0.0,
            **layer_stat,
        })
    if not layer_scores:
        raise RuntimeError("error-conditioned relation needs at least one server layer")
    aggregate = torch.stack(list(layer_scores.values()), dim=0).mean(dim=0)
    return layer_scores, aggregate.detach(), query_error_energy, {
        "formula": "R_raw[l,q,k]=relu(dot(g_pred-g_obs, O_pred-O_obs)/(||O_pred-O_obs||^2+eps)); R_norm=R_raw/mean_legal(R_raw); S=mean_l R_norm",
        "server_layers": [int(idx) for idx in layer_scores],
        "layer_count": len(layer_scores),
        "layer_stats": stats,
        "uses_real_o_proj": True,
        "uses_vector_contribution_not_value_norm_scalar": True,
        "uses_attention_sublayer_output_gate": True,
        "uses_layerwise_relational_scale_normalization": True,
        "uses_scalar_attention_weighted_activation_loss": False,
        "used": True,
    }
...
def compute_error_conditioned_patch_priorities(
    patches: Sequence[Sequence[int]],
    uncertain_positions: Set[int],
    uncertainty_scores: torch.Tensor,
    aggregate_score: torch.Tensor,
    variable_mask: torch.Tensor,
    query_error_energy: Optional[Dict[int, torch.Tensor]],
    top_r: int,
) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    variable = variable_mask.to(aggregate_score.device).bool()
    energy = None
    if query_error_energy:
        energy = torch.stack([v.to(aggregate_score.device).float() for _idx, v in sorted(query_error_energy.items())], dim=0).mean(dim=0)
    for patch_id, patch in enumerate(patches):
        patch = [int(pos) for pos in patch]
        up = patch_update_positions(patch, uncertain_positions)
        if not up:
            rows.append({"patch_id": patch_id, "positions": patch, "uncertain_positions": [], "uncertainty_score": 0.0, "relation_score": 0.0, "priority": 0.0, "processed": False, "skipped": True})
            continue
        uncertainty = float(uncertainty_scores[up].mean().detach().cpu())
        priority = 0.0
        relation_sum = 0.0
        for k in up:
            col = aggregate_score[:, int(k)].detach().float().clone()
            legal = variable.clone()
            legal[: int(k) + 1] = False
            col = torch.where(legal, col, torch.full_like(col, -float("inf")))
            finite = torch.isfinite(col)
            if int(finite.sum().detach().cpu()) == 0:
                continue
            vals, idx = torch.topk(col, k=min(int(top_r), int(finite.sum().detach().cpu())))
            for q, val in zip(idx.detach().cpu().tolist(), vals.detach().cpu().tolist()):
                if math.isfinite(float(val)) and int(q) > int(k):
                    e = float(energy[int(q)].detach().cpu()) if energy is not None and int(q) < int(energy.shape[0]) else 1.0
                    relation_sum += float(val)
                    priority += float(val) * e
        rows.append({"patch_id": patch_id, "positions": patch, "uncertain_positions": up, "uncertainty_score": uncertainty, "relation_score": relation_sum, "priority": priority, "processed": False, "skipped": False})
    max_priority = max([float(row["priority"]) for row in rows], default=0.0)
    if max_priority > 0:
        for row in rows:
            row["normalized_relation_score"] = float(row["priority"]) / max_priority
            row["priority"] = float(row["uncertainty_score"]) * float(row["normalized_relation_score"])
    else:
        for row in rows:
            row["normalized_relation_score"] = 0.0
    return rows
...
def build_patch_plan(
    ...
    min_patch: int,
    max_patch: int,
    query_error_energy: Optional[Dict[int, torch.Tensor]] = None,
    top_r: int = 4,
) -> Dict[str, Any]:
    method = canonical_method(method)
    identity = identity_aggregate_score(seq_len, relational_aggregate_score.device)
    patches = fixed_size_patches(seq_len, variable_mask, patch_size)
    if method in ERROR_REL_ORDER_METHODS:
        priority_rows = compute_error_conditioned_patch_priorities(patches, uncertain_positions, uncertainty_scores, relational_aggregate_score, variable_mask, query_error_energy, top_r)
        ordering_uses_relation = True
    else:
        priority_score = relational_aggregate_score if method in RELATIONAL_ORDER_METHODS else identity
        priority_rows = compute_patch_priorities(patches, uncertain_positions, uncertainty_scores, priority_score, variable_mask)
        ordering_uses_relation = method in RELATIONAL_ORDER_METHODS
    ordered_rows = sorted(priority_rows, key=lambda row: row["priority"], reverse=True) if ordering_uses_relation else list(priority_rows)
    return {
        "method": method,
        "patches": patches,
        "priority_rows": priority_rows,
        ...
        "patching_rule": f"fixed_size_{int(patch_size)}_from_sequence_start_with_tail_leftover",
        "partition_uses_relational_graph": False,
        "ordering_uses_relational_graph": ordering_uses_relation,
        "historical_relational_min_cut_partition_used": False,
        "priority_formula": "sum_q S[q,k] * ||e_attn[q]||^2 over TopR destinations" if method in ERROR_REL_ORDER_METHODS else "uncertainty/margin sequence order",
    }


def relational_attn_discrepancy(
    pred_evidence: Dict[str, Any],
    obs_evidence: Dict[str, Any],
    destinations_by_token: Dict[int, List[int]],
    updated_positions: Sequence[int],
    ...
    pred_out = {int(i): t for i, t in pred_evidence.get("attn_outputs", [])}
    obs_out = {int(i): t for i, t in obs_evidence.get("attn_outputs", [])}
    terms: List[torch.Tensor] = []
    for layer_index in sorted(set(pred_out) & set(obs_out)):
        dest_weight: Dict[int, float] = {}
        score = layer_scores.get(layer_index) if layer_scores else None
        for k in [int(x) for x in updated_positions]:
            for q in destinations_by_token.get(k, []):
                if score is not None and int(q) < score.shape[0] and int(k) < score.shape[1]:
                    val = float(score[int(q), int(k)].detach().cpu())
                else:
                    val = 1.0
                dest_weight[int(q)] = dest_weight.get(int(q), 0.0) + max(0.0, val)
        valid = [q for q in sorted(dest_weight) if q < pred_out[layer_index].shape[1]]
        if not valid:
            continue
        weights = torch.tensor([dest_weight[q] for q in valid], dtype=torch.float32, device=pred_out[layer_index].device)
        weights = weights / weights.sum().clamp_min(1e-12)
        diff = pred_out[layer_index][:, valid, :].float() - obs_out[layer_index][:, valid, :].to(pred_out[layer_index].device).float()
        per_q = diff.pow(2).sum(dim=-1).squeeze(0)
        terms.append((weights * per_q).sum())
    if not terms:
        first = pred_evidence.get("attn_outputs", [(0, torch.zeros(1))])[0][1]
        return first.sum() * 0.0
    return torch.stack(terms).mean()


def discrete_accept_error_rel_candidate(
    model: torch.nn.Module,
    cfg: RAPFinalConfig,
    current_z: torch.Tensor,
    candidate_z: torch.Tensor,
    ...
    variable_mask: torch.Tensor,
    fixed_public: Dict[int, int],
    obs_error_evidence: Dict[str, Any],
    destinations: Dict[int, List[int]],
    update_positions: Sequence[int],
    layer_scores: Dict[int, torch.Tensor],
    delta_cut_continuous: float,
    ...
    current_ids = ids_from_embeddings(current_z, embed_layer.weight, fixed_public)
    candidate_ids = ids_from_embeddings(candidate_z, embed_layer.weight, fixed_public)
    changed = variable_token_sequence_changed(current_ids, candidate_ids, variable_mask)
    if not changed:
        return False, "no_discrete_token_change", current_z.detach().clone(), {"discrete_token_changed": False, "token_change_count": 0, "delta_cut_continuous": float(delta_cut_continuous)}
    device = current_z.device
    current_disc = embeddings_from_ids(embed_layer, current_ids, device)
    candidate_disc = embeddings_from_ids(embed_layer, candidate_ids, device)
    token_change_count = sum(1 for pos, is_var in enumerate(variable_mask.detach().cpu().bool().tolist()) if is_var and pos < len(current_ids) and pos < len(candidate_ids) and int(current_ids[pos]) != int(candidate_ids[pos]))
    with torch.no_grad():
        current_hidden = capture_prefix_activation(model, cfg.target_layer, inputs_embeds=current_disc.to(dtype=embed_layer.weight.dtype), attention_mask=attention_mask)
        candidate_hidden = capture_prefix_activation(model, cfg.target_layer, inputs_embeds=candidate_disc.to(dtype=embed_layer.weight.dtype), attention_mask=attention_mask)
        current_cut = float(stage1_b0v_cut_loss(current_hidden, observed, variable_mask).detach().cpu())
        candidate_cut = float(stage1_b0v_cut_loss(candidate_hidden, observed, variable_mask).detach().cpu())
        current_evidence = server_forward_error_evidence(model, cfg.target_layer + 1, current_hidden.to(dtype=embed_layer.weight.dtype), attention_mask, False, True)
        candidate_evidence = server_forward_error_evidence(model, cfg.target_layer + 1, candidate_hidden.to(dtype=embed_layer.weight.dtype), attention_mask, False, True)
        current_rel = float(relational_attn_discrepancy(current_evidence, obs_error_evidence, destinations, update_positions, layer_scores).detach().cpu())
        candidate_rel = float(relational_attn_discrepancy(candidate_evidence, obs_error_evidence, destinations, update_positions, layer_scores).detach().cpu())
    delta_cut_disc = candidate_cut - current_cut
    delta_rel_disc = candidate_rel - current_rel
    stats = {
        "discrete_token_changed": True,
    ...
    if not math.isfinite(delta_cut_disc) or not math.isfinite(delta_rel_disc):
        return False, "invalid_candidate", current_z.detach().clone(), stats
    cut_ok = delta_cut_disc < -float(cfg.eps_cut)
    rel_ok = delta_rel_disc < -float(cfg.eps_rel)
    if not cut_ok and not rel_ok:
        return False, "both_not_improved", current_z.detach().clone(), stats
    if not cut_ok:
        return False, "cut_not_improved", current_z.detach().clone(), stats
    if not rel_ok:
        return False, "relation_not_improved", current_z.detach().clone(), stats
    return True, "accepted", candidate_disc.detach().clone(), stats
...
def run_sparse_repair(
    ...
    if aggregate_score is None:
        aggregate_score = identity_aggregate_score(int(z_stage1.shape[1]), device)
    uncertain_set = set(int(x) for x in uncertain_positions)
    plan = build_patch_plan(cfg.method, int(z_stage1.shape[1]), variable_mask, aggregate_score, uncertain_set, uncertainty_scores, cfg.patch_size, cfg.min_patch_size, cfg.max_patch_size, query_error_energy, cfg.top_r)
    priority_rows = plan["priority_rows"]
    ordered_rows = plan["ordered_rows"]
    destinations: Dict[int, List[int]] = {}
    destination_audit: Dict[str, Any] = {}
    if cfg.method in RELATION_EVIDENCE_METHODS:
        true_destinations = top_r_relational_destinations(layer_scores or {cfg.target_layer + 1: aggregate_score}, uncertain_positions, variable_mask, cfg.top_r)
        if cfg.method == "global_gate":
            destinations, destination_audit = deterministic_position_only_destinations(true_destinations, variable_mask)
        elif cfg.method == "shuffled_rel_matched":
            destinations, destination_audit = matched_shuffled_destinations(true_destinations, ordered_rows, uncertain_set, variable_mask, cfg.seed)
        else:
            destinations = true_destinations
            destination_audit = {"true_relational_destinations": True, "source_cardinality_matched": True, "patch_union_cardinality_matched": True, "destination_cardinality_agreement_rate": 1.0}
    else:
        destination_audit = {"relation_destinations_used": False}
    z = z_stage1.detach().clone()
    stage1_z = z_stage1.detach().clone()
    stage1_ids = ids_from_embeddings(stage1_z, embed_layer.weight, fixed_public)
    best_z = z.detach().clone()
    ...
    for repair_pass in range(max(1, int(cfg.max_repair_passes))):
        ...
        for row in ordered_rows:
            ...
            cut_before = float(global_cut_loss(model, cfg, z.to(dtype=embed_layer.weight.dtype), observed, attention_mask, variable_mask).detach().cpu())
            rel_before = 0.0
            if cfg.method in ERROR_REL_GATE_METHODS:
                with torch.no_grad():
                    h_before = capture_prefix_activation(model, cfg.target_layer, inputs_embeds=z.to(dtype=embed_layer.weight.dtype), attention_mask=attention_mask)
                    evidence_before = server_forward_error_evidence(model, cfg.target_layer + 1, h_before.to(dtype=embed_layer.weight.dtype), attention_mask, False, True)
                    rel_before = float(relational_attn_discrepancy(evidence_before, obs_states or {}, destinations, update_positions, None if cfg.method == "global_gate" else (layer_scores or {})).detach().cpu())
            candidate, candidate_stats = optimize_sparse_candidate(model, cfg, z, observed, attention_mask, variable_mask, fixed_public, update_positions)
            candidate_ids = ids_from_embeddings(candidate, embed_layer.weight, fixed_public)
            cut_after = float(global_cut_loss(model, cfg, candidate.to(dtype=embed_layer.weight.dtype), observed, attention_mask, variable_mask).detach().cpu())
            rel_after = rel_before
            if cfg.method in ERROR_REL_GATE_METHODS:
                with torch.no_grad():
                    h_after = capture_prefix_activation(model, cfg.target_layer, inputs_embeds=candidate.to(dtype=embed_layer.weight.dtype), attention_mask=attention_mask)
                    evidence_after = server_forward_error_evidence(model, cfg.target_layer + 1, h_after.to(dtype=embed_layer.weight.dtype), attention_mask, False, True)
                    rel_after = float(relational_attn_discrepancy(evidence_after, obs_states or {}, destinations, update_positions, None if cfg.method == "global_gate" else (layer_scores or {})).detach().cpu())
            delta_cut = cut_after - cut_before
            delta_rel = rel_after - rel_before
            discrete_stats: Dict[str, Any] = {}
            accepted_z = candidate
            if cfg.method in ERROR_REL_GATE_METHODS:
                accepted, reason, accepted_z, discrete_stats = discrete_accept_error_rel_candidate(model, cfg, z, candidate, observed, attention_mask, variable_mask, fixed_public, obs_states or {}, destinations, update_positions, {} if cfg.method == "global_gate" else (layer_scores or {}), delta_cut)
            elif cfg.method in OLD_REL_METHODS:
                accepted, reason, accepted_z, discrete_stats = OLD_DISCRETE_ACCEPT_CANDIDATE(model, cfg, z, candidate, observed, attention_mask, variable_mask, fixed_public, obs_states, destinations, update_positions, delta_cut)
            elif cfg.method in DELTA_CUT_ONLY_METHODS:
                accepted, reason = accept_candidate(delta_cut, 0.0, cfg.eps_cut, float("inf"))
            else:
                accepted, reason = accept_candidate(delta_cut, delta_rel, cfg.eps_cut, float(cfg.eps_rel))
            after_gate_z = rollback_if_rejected(z, accepted_z, accepted)
            after_gate_ids = ids_from_embeddings(after_gate_z, embed_layer.weight, fixed_public)
            z = after_gate_z
            if accepted:
            ...
            state_flow_stats = {
                "hash_stage1_ids": token_id_state_hash(stage1_ids),
                "hash_before_ids": token_id_state_hash(before_ids),
                "hash_candidate_ids": token_id_state_hash(candidate_ids),
                "candidate_hash_before_gate": token_id_state_hash(candidate_ids),
                "hash_after_gate_ids": token_id_state_hash(after_gate_ids),
                "hash_next_patch_start_ids": token_id_state_hash(after_gate_ids),
                "changed_positions_before_to_candidate": changed_positions_between(before_ids, candidate_ids, variable_mask),
                "changed_positions_before_to_after": changed_positions_between(before_ids, after_gate_ids, variable_mask),
                "changed_positions_after_to_next": [],
                "norm_current_z_minus_stage1_z_before": tensor_delta_norm(before_z, stage1_z),
                "norm_after_gate_z_minus_stage1_z": tensor_delta_norm(after_gate_z, stage1_z),
                "norm_current_z_after_accept_minus_current_z_before": tensor_delta_norm(after_gate_z, before_z) if accepted else 0.0,
            }
            event = {"repair_pass": repair_pass + 1, "patch_id": int(row["patch_id"]), "accepted": bool(accepted), "rejected_reason": reason, "delta_cut": delta_cut, "delta_rel": delta_rel, "relative_delta_rel": (delta_rel / rel_before) if rel_before > 0 else None, "cut_before": cut_before, "cut_after": cut_after, "rel_before": rel_before, "rel_after": rel_after, "discrete_acceptance_gate": cfg.method in DISCRETE_REL_GATE_METHODS, **audit, **candidate_stats, **discrete_stats, **state_flow_stats}
            events.append(event)
            pr_row["accepted"] = bool(accepted)
            pr_row["rejected_reason"] = reason
            processed_priority.append(pr_row)
...
def invert_observed(
    ...
    if cfg.method == "variable_only_uniform":
        ...
        with torch.no_grad():
            stage1_hidden = capture_prefix_activation(model, cfg.target_layer, inputs_embeds=z.to(dtype=embed_layer.weight.dtype), attention_mask=attention_mask)
            residual = activation_residual_by_position(stage1_hidden, observed_activation)
        _scores, _positions, uncertainty_stats = build_uncertainty_score(margins, variable_audit.variable_mask, cfg.uncertainty_fraction, activation_residual=residual, detector=cfg.uncertainty_detector)
    if cfg.method in SPARSE_METHODS:
        embed_layer = model.get_input_embeddings()
        margins = token_margins(z, embed_layer.weight)
        with torch.no_grad():
            stage1_hidden = capture_prefix_activation(model, cfg.target_layer, inputs_embeds=z.to(dtype=embed_layer.weight.dtype), attention_mask=attention_mask)
            residual = activation_residual_by_position(stage1_hidden, observed_activation)
        uncertainty_scores, uncertain_positions, uncertainty_stats = build_uncertainty_score(margins, variable_audit.variable_mask, cfg.uncertainty_fraction, activation_residual=residual, detector=cfg.uncertainty_detector)
        layer_scores = None
        aggregate = None
        obs_relation_state: Optional[Any] = None
        query_error_energy = None
        if cfg.method in RELATION_EVIDENCE_METHODS:
            with torch.no_grad():
                if cfg.method in ERROR_REL_METHODS:
                    obs_evidence = server_forward_error_evidence(model, cfg.target_layer + 1, observed_activation.to(dtype=next(model.parameters()).dtype), attention_mask, True, True)
                    pred_evidence = server_forward_error_evidence(model, cfg.target_layer + 1, stage1_hidden.to(dtype=next(model.parameters()).dtype), attention_mask, True, True)
                    layer_scores, aggregate, query_error_energy, graph_stats = build_error_conditioned_relational_graph(model, cfg.target_layer + 1, obs_evidence, pred_evidence, variable_audit.variable_mask)
                    if cfg.method == "shuffled_rel_matched":
                        graph_stats.update({"shuffled_matched_control": True, "shuffle_applied_at_destination_level": True})
                    obs_relation_state = obs_evidence
                else:
                    obs_states, attentions, value_norms = server_forward_evidence(model, cfg.target_layer + 1, observed_activation.to(dtype=next(model.parameters()).dtype), attention_mask, True, True)
                    layer_scores, aggregate, graph_stats = OLD_BUILD_RELATIONAL_GRAPH(attentions, value_norms, variable_audit.variable_mask)
                    obs_relation_state = obs_states
                graph_stats["used"] = True
        z, repair_stats, repair_events, priority_rows, rel_extra = run_sparse_repair(model, cfg, observed_activation, z, variable_audit.variable_mask, fixed_public, layer_scores, aggregate, obs_relation_state, uncertain_positions, uncertainty_scores, query_error_energy)
    embed_layer = model.get_input_embeddings()
    recovered_ids = naive_discretization(z, embed_layer.weight)
    for pos, token_id in fixed_public.items():
        if 0 <= int(pos) < len(recovered_ids):
    ...
    with torch.no_grad():
        ...
        var_loss = stage1_b0v_cut_loss(hidden, observed_activation, variable_audit.variable_mask)
        active = all_loss if cfg.method == "original_pia_baseline" else var_loss
    losses.update({"activation_loss": float(active.detach().cpu()), "all_token_uniform_loss": float(all_loss.detach().cpu()), "variable_uniform_loss": float(var_loss.detach().cpu()), "optimization_loss": float(active.detach().cpu()), "loss_mode": loss_mode if cfg.method in BASELINE_METHODS else "sparse_patch_repair"})
    audits = {
        "variable_mask_audit": variable_mask_audit_payload(variable_audit),
        "uncertainty_stats": uncertainty_stats,
        "relational_graph_stats": graph_stats,
        ...
        "acceptance_gate_stats": {"events": repair_events, **repair_stats},
        "relational_destinations": rel_extra,
        "strict_top1": {"top_k_embedding": cfg.top_k_embedding, "top_y_semantic": cfg.top_y_semantic, "semantic_speculation": cfg.semantic_speculation, "adaptive_discretization": cfg.adaptive_discretization},
        "attack_receives_ground_truth": False,
        **method_integrity_audit(),
    }
    return recovered_ids, losses, audits, history + repair_events, repair_stats
...
def validate_method_integrity(checked: Optional[Sequence[Any]] = None) -> Dict[str, Any]:
    checked = list(checked or [invert_observed, run_sparse_repair, optimize_sparse_candidate, build_error_conditioned_relational_graph, top_r_relational_destinations, relational_attn_discrepancy])
    dummy_terms = {"dummy", "dummy_attention", "alpha_init"}
    scalar_call_terms = {"weighted_activation_loss", "build_mts_token_weights", "build_attention_scale_alpha", "attention_weighted_residual_loss", "attention_scaled_activation_loss"}
    dummy_hits: List[Dict[str, str]] = []
    scalar_hits: List[Dict[str, str]] = []
    for fn in checked:
        tree = ast.parse(textwrap.dedent(inspect.getsource(fn)))
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Call):
                ...
                if leaf in scalar_call_terms:
                    scalar_hits.append({"function": fn.__name__, "call": call_name})
    return {"checked_functions": [fn.__name__ for fn in checked], "dummy_reference_hits": dummy_hits, "scalar_attention_call_hits": scalar_hits, "passes": not dummy_hits and not scalar_hits}


def method_integrity_audit() -> Dict[str, Any]:
    check = validate_method_integrity()
    return {"uses_dummy_attention": bool(check["dummy_reference_hits"]), "uses_scalar_attention_weighted_activation_loss": bool(check["scalar_attention_call_hits"]), "method_integrity_check": check, "attention_role": "relation-isolation controls for routing/order/downstream consistency gate"}

def validate_attack_api() -> Dict[str, Any]:
    checked = [invert_observed, optimize_stage1, run_sparse_repair, optimize_sparse_candidate, build_error_conditioned_relational_graph]
...
def verify_no_leakage(args: argparse.Namespace) -> Dict[str, Any]:
    cfg = config_from_args(args, "TRUE_REL_FULL", "verify", str(Path(args.output_root) / "leakage_verification" / "sample"))
    cfg.dataset_len = 1
    require_strict_top1(cfg)
    tokenizer, model, device = load_model_and_data(cfg)
    ...
    with torch.no_grad():
        ...
        probe = observed.detach().clone().float()
        probe[:, 1:, :] = probe[:, 1:, :] + 0.01
        obs_error_evidence = server_forward_error_evidence(model, cfg.target_layer + 1, observed.to(dtype=next(model.parameters()).dtype), attention_mask, True, True)
        pred_error_evidence = server_forward_error_evidence(model, cfg.target_layer + 1, probe.to(dtype=next(model.parameters()).dtype), attention_mask, True, True)
        obs_states = obs_error_evidence["states"]
    fixed_public = inferred_boundary_special_tokens(tokenizer, int(input_ids.shape[1]))
    variable_audit = build_variable_mask(attention_mask, fixed_public, getattr(tokenizer, "all_special_ids", None), token_ids=None)
    layer_scores, aggregate, query_error_energy, graph_stats = build_error_conditioned_relational_graph(model, cfg.target_layer + 1, obs_error_evidence, pred_error_evidence, variable_audit.variable_mask)
    shuffled_layer_scores, shuffled_aggregate, shuffle_stats = shuffle_layer_scores(layer_scores, aggregate, variable_audit.variable_mask, cfg.seed)
    ...
    priority = compute_error_conditioned_patch_priorities(patches, set(uncertain), scores, aggregate, variable_audit.variable_mask, query_error_energy, cfg.top_r)
    ...
    legal = legal_variable_causal_mask(aggregate, variable_audit.variable_mask)
    legal_values = aggregate[legal]
    shuffled_values = shuffled_aggregate[legal]
    relation_nonzero = bool(int((legal_values > 0).sum().detach().cpu()) > 0)
    relation_has_nan = has_nan(graph_stats) or bool(torch.isnan(aggregate).any().detach().cpu())
    shape_ok = all(tuple(t.shape[:2]) == (1, int(input_ids.shape[1])) for _idx, t in obs_error_evidence["attn_outputs"])
    scale_gap = abs(float(legal_values.mean().detach().cpu()) - float(shuffled_values.mean().detach().cpu())) if int(legal.sum().detach().cpu()) else 0.0
    result = {
        "title": EXPERIMENT_TITLE,
        "dataset_meta": dataset_meta,
        "target_layer": cfg.target_layer,
        ...
        "o_proj_attention_output_shape_ok": shape_ok,
        "relation_score_nonzero": relation_nonzero,
        "relation_score_has_nan": relation_has_nan,
        "shuffled_scale_gap": scale_gap,
        "shuffled_scale_preserved": scale_gap < 1e-5,
        "passes": boundary_diff < 5e-3 and api_check["passes"] and integrity["passes"] and subset_ok and shape_ok and relation_nonzero and not relation_has_nan and scale_gap < 1e-5,
    }
    out = Path(args.output_root) / "leakage_verification"
    out.mkdir(parents=True, exist_ok=True)
    json_dump(out / "leakage_verification.json", result)
    Path("analysis/rap_error_rel_preflight.md").write_text("# RAP-ERROR-REL Preflight Audit\n\n```json\n" + json.dumps(result, indent=2, ensure_ascii=True) + "\n```\n", encoding="utf-8")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-patch-size", type=int, default=16)
    parser.add_argument("--max-patch-size", type=int, default=16)
...
def main() -> None:
    ...
    if args.mode == "verify":
        ...
        run_method_set(args, ["B0V", "SPARSE_FIXED", "GLOBAL_GATE", "TRUE_REL_GATE_ONLY", "TRUE_REL_ORDER_ONLY", "TRUE_REL_FULL", "SHUFFLED_REL_MATCHED"], "smoke")
    elif args.mode == "dev":
        run_method_set(args, ["B0V", "SPARSE_FIXED", "GLOBAL_GATE", "TRUE_REL_GATE_ONLY", "TRUE_REL_ORDER_ONLY", "TRUE_REL_FULL", "SHUFFLED_REL_MATCHED"], f"dev_seed{args.seed}")
